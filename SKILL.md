...
"$PY" "$SK" 文章.md --wait-ip 900             # 6) 等白名单生效再自动上传（最多 900s）
```
...
2. **封面**：必须有封面，否则脚本直接退出。流程：
   ...
   - AI 生图常带角标水印（右上/右下/最底部都出现过），用 Pillow 中心裁切到
     **2.35:1（推荐 1080×460）** 去掉。
     ⚠️ **别想当然认为裁不掉——先算水印在不在裁切区内**：
     ```
     裁切高 ch = W / 2.3478，中心裁切覆盖 y ∈ [(H-ch)//2, (H-ch)//2 + ch]
     ```
     实测 1536×1024 → ch=654、覆盖 y 185–839。水印若落在**最底部（y≈960+）**，
     就在区外、**天然被裁掉，不需要修补**；只有落在区内（如画面纵向 60% 处）
     才走下方「水印修补」。
     判断办法：裁完把右下角 `crop((940,265,1080,345))` 放大看一眼。
...
```
"$PY" "$SK" --show-ip
  ▶ 微信实际看到: 223.104.80.205
  请加入：223.104.80.205  /  223.104.80.0/24  /  223.104.0.0/16
```

**第二条：填网段，而且要填得足够宽。**
公众号 API IP 白名单**支持 CIDR**（官方文档示例 `172.0.0.1/24`；
不支持 `172.0.0.*` 星号写法）。但**`/24` 往往不够** —— 实测该网络（移动湛江 CGNAT）
的微信侧出口会跨 /24 漂移，一小时内在 `223.104.72.x → 223.104.88.x → 223.104.80.x`
之间反复跳。所以 **直接加 `223.104.0.0/16`，一次到位**，别一层层试。
（`/16` 确实放宽了范围，但真正的密钥是 AppSecret；白名单只是纵深防御的一层。
介意范围太宽就走长期方案。）

**第三条：加完不用手跑第二遍。**
白名单有几分钟生效延迟，用 `--wait-ip` 让它自己等：

```
"$PY" "$SK" 文章.md --wait-ip 900      # 最多等 900s，探测通过就自动上传
```
探测走的是只读的 token 接口，**不会产生任何草稿或素材**，可以放心挂着。
